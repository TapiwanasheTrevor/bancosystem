...
@section('title', 'Team Commission Report')

@section('content')
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Team Commission Report</h1>
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        <form action="{{ route('commissions.team-report') }}" method="GET" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div class="md:col-span-2">
                    <label for="team_lead_id" class="block text-sm font-medium text-gray-700 mb-1">Select Team Lead</label>
                    <select id="team_lead_id" name="team_lead_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                        <option value="">Select Team Lead</option>
                        @foreach($teamLeads as $lead)
                            <option value="{{ $lead->id }}" {{ (isset($teamLead) && $teamLead->id == $lead->id) ? 'selected' : '' }}>
                                {{ $lead->name }}
                            </option>
                        @endforeach
                    </select>
    ...
    @if(isset($teamLead))
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden mb-6">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        <h3 class="text-sm font-medium text-gray-500">Team Lead</h3>
                        <p class="text-lg font-semibold">{{ $teamLead->name }}</p>
                    </div>
                    <div>
                        <h3 class="text-sm font-medium text-gray-500">Members</h3>
                        <p class="text-lg font-semibold">{{ count($agentPerformance) }} Agents</p>
                    </div>
                    <div>
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden mb-6">
            <div class="px-6 py-4 bg-indigo-700 text-white">
                <h2 class="text-lg font-semibold">Agent Performance</h2>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total Transactions</th>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">% of Team Total</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($agentPerformance as $agentId => $performance)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $performance['name'] }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $performance['count'] }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-indigo-600">${{ number_format($performance['total'], 2) }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-green-600">${{ number_format($performance['paid'], 2) }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-blue-600">${{ number_format($performance['approved'], 2) }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-yellow-600">${{ number_format($performance['pending'], 2) }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="w-full bg-gray-200 rounded-full h-2.5">
                                        <div class="bg-indigo-600 h-2.5 rounded-full" style="width: {{ $totalCommission > 0 ? ($performance['total'] / $totalCommission * 100) : 0 }}%"></div>
                                    </div>
                                    <div class="text-xs text-gray-500 mt-1">
                                        {{ $totalCommission > 0 ? number_format(($performance['total'] / $totalCommission * 100), 1) : 0 }}%
                                    </div>
                                </td>
                            </tr>
                        @endforeach
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                                    No commission transactions found for this team in the selected period.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    @else
        <div class="bg-white rounded-lg shadow p-6 text-center">
            ...
            <h2 class="text-xl font-semibold text-gray-700 mb-2">Select a Team Lead to Generate Report</h2>
            <p class="text-gray-500">Choose a team lead from the dropdown above and click 'Generate Report' to view team commission data.</p>
        </div>
    @endif
</div>
